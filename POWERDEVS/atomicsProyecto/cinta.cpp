...
void cinta::dint(double t) {
        if((!(cJ.empty()) && !(cPc.empty()) && potencia(cJ,time) > potencia(cPc,time))) { //gana colision jugador
                ...
                cJ.front().first = pesoNuevo(cJ,cPc,time);  //se actualiza el peso del objeto de la cabeza de la cola ganadora
                ...
                cPc.front().first = pesoNuevo(cPc,cJ,time); //se actualiza el peso del objeto de la cabeza de la cola ganadora
                sigma = calcularsigma(cJ,cPc,time); //se calcula el nuevo sigma
